...
//				printf("Unhandled tag: %d\n",ifds[index].tag);
	...
	if( noDataDef != 1 ) {
		...
		}else if(datatype == LONG_TYPE)
		{
			...
			if(rank==0)printf("The value: %ld will be used as representing missing data.\n",*((long*)nodata));
		}
		else
		{
			nodata = new float;
...
tiffIO::tiffIO(char *fname, DATA_TYPE newtype, void* nd, const tiffIO &copy) {
    ...
    MPI_File_set_errhandler(fh, MPI::ERRORS_THROW_EXCEPTIONS);
...
void tiffIO::write(long xstart, long ystart, long numRows, long numCols, void* source) {
	...
	if(rank==0) {
		...
		if ( version == TIFF ) {
			...
			MPI_File_write( fh, zeros, 4, MPI_BYTE, &status);  //DGT
		}
		else {
			MPI_File_write( fh, zeros, 8, MPI_BYTE, &status);   //DGT
		}
